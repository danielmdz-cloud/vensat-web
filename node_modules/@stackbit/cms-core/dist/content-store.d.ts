...
import { UserCommandSpawner, CommandRunner, DocumentSpecifier } from '@stackbit/types';
import { Config, ImageModel, Model, Preset, PresetMap } from '@stackbit/sdk';
import * as ContentStoreTypes from './types';
import { SearchFilter } from './types';
import { GitService } from './services';
export type HandleConfigAssets = <T extends Model>({ models, presets }: {
    models?: T[];
    presets?: PresetMap;
}) => Promise<{
    models: T[];
...
export declare class ContentStore {
    ...
    }): Record<string, Record<string, Record<string, Model | ImageModel>>>;
    ...
    getRunningCustomActionsForUser(getRunningActionsRequest: ContentStoreTypes.APIGetRunningCustomActionsRequest): Promise<ContentStoreTypes.APICustomAction[]>;
    runCustomAction(runActionRequest: ContentStoreTypes.APIRunCustomActionRequest): Promise<ContentStoreTypes.APIRunCustomActionResponse>;
    getPresets({ locale }?: {
        locale?: string;
    }): Record<string, any>;
    ...
    getCSIDocuments({ documentSpecs, srcType, srcProjectId, limit, offset }?: {
        documentSpecs?: DocumentSpecifier[];
        srcType?: string;
        srcProjectId?: string;
        limit?: number;
    ...
    getStagedChanges({ scope, locale, shallow, objects, user }: {
    ...
    getApiAssetsV2({ assetSpecs, user, pageSize, pageNum, searchQuery, locale }?: {
        srcType?: string;
        srcProjectId?: string;
        pageSize?: number;
        pageNum?: number;
    ...
    updateAsset({ srcType, srcProjectId, srcAssetId, updateOperations, user }: {
        srcType: string;
        srcProjectId: string;
        srcAssetId: string;
        updateOperations: ContentStoreTypes.UpdateOperation[];
    ...
    archiveDocument({ srcType, srcProjectId, srcDocumentId, user }: {
        srcType: string;
        srcProjectId: string;
        srcDocumentId: string;
        user?: ContentStoreTypes.User;
    }): Promise<void>;
    unarchiveDocument({ srcType, srcProjectId, srcDocumentId, user }: {
        srcType: string;
        srcProjectId: string;
        srcDocumentId: string;
        user?: ContentStoreTypes.User;
    ...
    getDocumentVersions({ srcType, srcProjectId, documentId, locale, user }: {
        srcType: string;
        srcProjectId: string;
        documentId: string;
        locale?: string;
    ...
    getDocumentForVersion({ srcType, srcProjectId, documentId, versionId, locale, user }: {
        srcType: string;
        srcProjectId: string;
        documentId: string;
        versionId: string;
    ...
    unpublishDocuments({ objects, user }: {
        objects: {
            srcType: string;
            srcProjectId: string;
            srcObjectId: string;
